...
@cli.command()
@click.argument('text')
@click.argument('key_text')
@click.option('--decode', '-d', is_flag=True, help='Perform decoding')
def aes_encrypt(text, key_text, decode):
    """
    AES算法 字符串加密/解密
    """
    digest = (hashlib.sha256(key_text.encode())).digest()
    key = base64.urlsafe_b64encode(digest)
    # key = Fernet.generate_key() # 也可以生成随机的 Fernet 密钥
    output = ''
    if decode:
        output = Fernet(key).decrypt(text.encode('utf-8')).decode('utf-8')
    else:
        output = Fernet(key).encrypt(text.encode('utf-8')).decode('utf-8')
